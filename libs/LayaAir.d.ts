...
declare module laya.d3.core {
	...
	class Transform3D extends laya.events.EventDispatcher  {
		...
		static pointToLocalFrame(position: Laya.Vector3, quaternion: Laya.Quaternion, worldPoint: Laya.Vector3, result?: Laya.Vector3):Laya.Vector3;
		pointToLocalFrame(worldPoint:Laya.Vector3, result?:Laya.Vector3):Laya.Vector3;

		static pointToWorldFrame(position: Laya.Vector3, quaternion: Laya.Quaternion, localPoint: Laya.Vector3, result?: Laya.Vector3):Laya.Vector3;
		pointToWorldFrame(localPoint:Laya.Vector3, result?:Laya.Vector3):Laya.Vector3;

		static vectorToWorldFrame(quaternion: Laya.Quaternion, localVector: Laya.Vector3, result?: Laya.Vector3):Laya.Vector3;
		vectorToWorldFrame(localVector:Laya.Vector3, result?:Laya.Vector3):Laya.Vector3;

		static vectorToLocalFrame(quaternion: Laya.Quaternion, worldVector: Laya.Vector3, result?: Laya.Vector3):Laya.Vector3;
		vectorToLocalFrame(worldVector:Laya.Vector3, result?:Laya.Vector3):Laya.Vector3;
...
declare module laya.d3.math {
	...
	class Color implements laya.d3.core.IClone  {
		...
		toVector3(target?:Laya.Vector3):Laya.Vector3;
		toVector4(target?:Laya.Vector4):Laya.Vector4;
...
declare module laya.d3.math {
	...
	class Matrix4x4 implements laya.d3.core.IClone  {
		...
		copyPosition(m:Matrix4x4):Matrix4x4;
		extractBasis(xAxis: Laya.Vector3, yAxis: Laya.Vector3, zAxis: Laya.Vector3):Matrix4x4;
		makeBasis(xAxis: Laya.Vector3, yAxis: Laya.Vector3, zAxis: Laya.Vector3):Matrix4x4;
		extractRotation(m:Matrix4x4):Matrix4x4;
		makeRotationFromEuler(euler:Vector3):Matrix4x4;
		makeRotationFromQuaternion(q:Quaternion):Matrix4x4;
		lookAt(eye: Laya.Vector3, target: Laya.Vector3, up: Laya.Vector3):Matrix4x4;
		multiply(m:Matrix4x4):Matrix4x4;
		premultiply(m:Matrix4x4):Matrix4x4;
		multiplyMatrices(a:Matrix4x4, b:Matrix4x4):Matrix4x4;
		multiplyScalar(s:number):Matrix4x4;
		determinant():number;
		getInverse(m:Matrix4x4):Matrix4x4;
		...
		makeRotationZ(theta: number):Matrix4x4;
		makeRotationAxis(axis: Laya.Vector3, angle: number):Matrix4x4;
		makeScale(x: number, y: number, z: number):Matrix4x4;
		makeShear(x: number, y: number, z: number):Matrix4x4;
		compose(position: Laya.Vector3, quaternion: Laya.Quaternion, scale: Laya.Vector3):Matrix4x4;
		decompose(position: Laya.Vector3, quaternion: Laya.Quaternion, scale: Laya.Vector3):Matrix4x4;
		equals(m:Matrix4x4):boolean;
...
declare module laya.d3.math {
	...
	class Quaternion implements laya.d3.core.IClone  {
		...
		copy(s?: Quaternion): Quaternion;
		toArray():number[];
		setFromEuler(euler:Laya.Vector3):Quaternion;
		setFromAxisAngle(axis:Laya.Vector3, angle:number):Quaternion;
		setFromRotationMatrix(m:Laya.Matrix4x4):Quaternion;
		setFromUnitVectors(vFrom:Laya.Vector3, vTo:Laya.Vector3):Quaternion;
...
declare module laya.d3.math {
	...
	class Vector2 implements laya.d3.core.IClone  {
		...
		// extension start
		vsub(v:Vector2, target?:Vector2):Vector2;
		vadd(v:Vector2, target?:Vector2):Vector2;
		mult(n:number, target?:Vector2):Vector2;
		divide(n:number, target?:Vector2):Vector2;
		set(newX:number, newY:number);
		lerp(a:Vector2, t:number, target?:Vector2):Vector2;
		lerpUnclamped(a:Vector2, t:number, target?:Vector2):Vector2;
		magnitude():number;
		normalize();
		unit();
		...
		distanceSquared(p:Vector2):number;
		negate(target?:Vector2):Vector2;
		copy(v:Vector2):Vector2;
		toArray():number[];
		fromArray(arr:number[]):Vector2;
		almostEquals(v: Laya.Vector2, precision?: number):boolean;
		isZero():boolean;
		min(v:Vector2):Vector2;
		max(v:Vector2):Vector2;

		static moveTowards(current: Vector2, target: Vector2, maxDistanceDelta: number, out?: Vector2):Vector2;
		static angle(from: Laya.Vector2, to: Laya.Vector2):number;
		static signedAngle(from: Laya.Vector2, to: Laya.Vector2):number;
		static reflect(inDirection: Vector2, inNormal: Vector2, target?:Vector2):Vector2;
		static zero:Vector2;
		static one:Vector2;
		static up:Vector2;
		static down:Vector2;
...
declare module laya.d3.math {
	...
	class Vector3 implements laya.d3.core.IClone  {
		...
		static angle(ma:Vector3, mb:Vector3):number;
		 ...
		 * @param offset 数组偏移。
		 */
		fromArray(array:any[],offset?:number):any;
		...
		cross(v: Laya.Vector3, target?: Laya.Vector3):Vector3;
		...
		divide(scalar:number, target?:Vector3):Vector3;
		...
		isZero():boolean;
		isAntiparallelTo(v: Laya.Vector3, precision: number):boolean;
		min(v:Vector3):Vector3;
		max(v:Vector3):Vector3;
		applyMatrix3(m:Matrix3x3):Vector3;
		applyMatrix4(m:Matrix4x4):Vector3;
		applyQuaternion(q:Quaternion):Vector3;
		setFromMatrixScale(m: Laya.Matrix4x4):Vector3;
		setFromMatrixPosition(m: Laya.Matrix4x4):Vector3;
		setFromMatrix3Column(m: Laya.Matrix4x4, index: number):Vector3
		setFromMatrixColumn(m:Matrix4x4, index:number):Vector3;
		
		static moveTowards(current: Vector3, target: Vector3, maxDistanceDelta: number, out?: Vector3):Vector3;
		static reflect(inDirection:Vector3, inNormal:Vector3, target?:Vector3):Vector3;
...
declare module laya.d3.resource.models {
	...
	class PrimitiveMesh  {
		...
		static _createMesh(dec: laya.d3.graphics.VertexDeclaration, vertices: Float32Array, indices: Uint16Array): laya.d3.resource.models.Mesh;
	}
...
new Animation_Example();
	...
	class Animation extends laya.display.AnimationBase  {
		 ...
		 * <p>动画模版缓存池是以一定的内存开销来节省CPU开销，当相同的动画模版被多次使用时，相比于每次都创建新的动画模版，使用动画模版缓存池，只需创建一次，缓存之后多次复用，从而节省了动画模版创建的开销。</p>
		 * <p>因为返回值为Animation对象本��，所以可以使用如下语法：loadAtlas(...).loadAtlas(...).play(...);。</p>
...
获取端点列表。
		...
		drawCircleWithAngle(x:number, y:number, radius:number, startAngle:number, endAngle:number, offset:number, clockwise:boolean, fillColor:string, lineColor?:string, lineWidth?:number);
	}
...
declare module laya.display {
	...
	class Node extends laya.events.EventDispatcher  {
		...
		/**
		 * 定时执���某函数一次。功能同Laya.timer.timerOnce()。
...
declare module laya.net {
	...
	class LoaderManager extends laya.events.EventDispatcher  {
		 ...
		 * @param complete 加载结束回调。根据url类型不同分为2种情况：1. url为String类型，也就是单个资源地址，如果加载成功，则回调参数值为加载完成的资源，���则为null；2. url为数组类型，指定了一组要加载的资源，如果全部加载成功，则回调参数值为true，否则为false。
...
<p><b>注意：</b>返回的 Int16Array 对象，在 JavaScript 环境下，是原生的 HTML5 Int16Array 对象，对此对象的读取操作都是基于运行此程序的当前主机字节序，此顺序可能与实际数据的字节序不同，如果使用此对象进行读取，需要用户知晓实际数据的字节序和当前主机字节序，如果相同，可正常读取，否则需要用户对实际数据(Int16Array.buffer)包装一层 DataView ，使用 DataView 对象可按照指定的字节序进行读取。</p>
		...
		/**
		 * 清除字节数组的内容，并将 length 和 pos 属��重置为 0。调用此方法将释放 Byte 实例占用的内存。
		 */
		clear():void;
